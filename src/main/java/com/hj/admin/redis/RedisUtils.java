...
public class RedisUtils {
    ...
    //map----json字符串---->对象
    public <T>T hmgetObject(String key,Class<T> tClass) throws JsonProcessingException {
        Map<Object, Object> hmget = hmget(key);
        if(CollectionUtils.isEmpty(hmget)) return null;
        //查询到了 先把数据转成json字符串
        String s = JsonUtils.objToJson(hmget);
        //再把json字符串转回对象
        return  JsonUtils.jsonToObj(s,tClass);
    }
